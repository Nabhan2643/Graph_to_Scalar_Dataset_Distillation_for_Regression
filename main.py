...
CFG = dict(
    ...
    pge_hidden_dim=128,
    pge_layers=3,
    pge_threshold=0.5,      # threshold for PGE edge selection
...
for _ in range(CFG["syn_graphs"]):
    ...
    # Initialize with empty edge index (will be generated by PGE)
    edge_index_syn = torch.zeros(
        (2, 0),
        dtype=torch.long,
        device=device
    )
    ...
    train_syn.append(
        GraphData(X=X_syn, edge_index=edge_index_syn, y=y_syn, requires_grad=True)
    )
...
# Set PGE threshold
mlp.threshold = CFG["pge_threshold"]

print("✔ MLP trained/loaded and threshold set")
